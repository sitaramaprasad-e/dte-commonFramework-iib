    ...
    stages {
       ...
       // Build BAR file from source code
        ...
        stage('Override and Deploy BAR') {
         steps {
                sh "/buildconductor/iib/run-automation.sh overrideAndDeploy commonframework na na na TEST webuser1 passw0rd RetryApplication overrides/RetryApplication-ST.properties ../brokerFiles-load/DEV/TEST.broker"
